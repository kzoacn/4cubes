...
structured four-cube identity into generalized Pell equations
$u^2-d v^2=M$.  We also give a heuristic model, based on the square tests
forced by primes dividing $d$ and on conditional Bernays-type density, which
explains the observed scale of the search parameter.  The same method also
quickly finds representations for large integers, such as
$n=9\cdot 2^{128}+5$, within seconds.
\end{abstract}
...
\eqref{eq:main_problem} are allowed to be negative.
...
classes the search can be very hard.  Booker resolved the notable case
$33$~\cite{Booker2019}, and Booker and Sutherland resolved
$42$~\cite{BookerSutherland2021}.
...
\subsubsection{Sum of four cubes}
...
explicit integrality and parity conditions.  Second, we use this family to
produce certificates for every $1\leq n\leq 10^9$.  Third, we provide an
independently checkable verification protocol and a heuristic model for the
...
The computation described in Section~\ref{sec:experimental} produced, for
every integer $n$ with $1\leq n\leq 10^9$, an explicit representation
\[
        x_1^3+x_2^3+x_3^3+x_4^3=n.
\]
Each recorded quadruple was checked using exact integer arithmetic, and the
verification protocol is described in Section~\ref{subsec:verification_protocol}.

\subsection{Transformation to Pell equation}
\label{subsec:transformation}

\begin{theorem}[Pell transformation with parity conditions]
...
The Pell step in Algorithm~\ref{alg:main} means that we do not rely on a
single arbitrary solution returned by a generalized Pell solver.  The
reference implementation enumerates all representatives returned by the
underlying quadratic-form solver, keeps those satisfying
...
which generalized Pell solvers are available.

\section{Heuristic for the search parameter}
\label{sec:heuristic}

\subsection{Square tests for candidate parameters}
\label{subsec:square_tests}

The values $M_{n,d}=(4n-1+d^3)/3$ are not random integers independent of
$d$.  In particular, Lemma~\ref{lem:local_obstruction} shows that a prime
dividing $d$ can force the success probability for that $d$ to be zero.  We
therefore separate these necessary square tests from the global
...
\[
        ...
        d\equiv 1-n\pmod6,
\]
and, for every prime power $p^\alpha\Vert d$, the integer $M_{n,d}$ is a
square modulo $p^\alpha$.  Otherwise set $S_n(d)=0$.  Only the values with
$S_n(d)=1$ pass the square tests used before the Pell search.

For odd primes $p\neq3$ with $p\nmid 4n-1$, the square test at a prime
factor $p$ of $d$ is controlled by the quadratic character
\[
        \chi_n(p)=
        \left(\frac{(4n-1)\cdot 3^{-1}}{p}\right).
\]
Thus a prime $p$ with $\chi_n(p)=-1$ cannot divide a parameter $d$ with
$S_n(d)=1$.  For typical $n$, and away from the finitely many primes
dividing $6(4n-1)$, these characters are modeled as taking the values
$\pm1$ with roughly equal frequency.

\subsection{Counting viable parameters}
\label{subsec:counting_viable}

The preceding character model suggests that the prime factors of a
parameter passing the square tests must lie in a set of primes of natural
density about $1/2$.  Standard sieve heuristics for integers supported on
such a set then predict
\[
        \#\{d\leq t:S_n(d)=1\}
        \approx C_n \frac{t}{\sqrt{\log t}},
\]
where $C_n$ depends on the exceptional behavior at small primes and at
primes dividing $4n-1$.  This is the point at which the dependence of
$M_{n,d}$ on $d$ enters the model: values of $d$ failing the square tests
are removed before any Bernays-type density is invoked.
...
$X$ represented by a fixed indefinite form, after the necessary square tests
are taken into account.  We use this only as a conditional heuristic.  Given
$S_n(d)=1$, model the probability that
\[
        u^2-d v^2=M_{n,d}
\]
has a solution in the required parity class by
\[
        \Pr(\text{success at }d\mid S_n(d)=1)
        \approx \frac{K_n(d)}{\sqrt{\log M_{n,d}}},
\]
where $K_n(d)$ is treated as a slowly varying factor.  During the early part
of the search, when $d^3$ is small compared with $n$, we have
$\log M_{n,d}\asymp\log n$.

The expected number of successful candidates up to $t$ is then modeled by
\[
        \Lambda_n(t)
        \approx
        \sum_{\substack{d\leq t\\ S_n(d)=1}}
...
stopping-time model used to interpret the empirical distribution in
Section~\ref{subsec:distribution}.

\section{Experimental Verification}
\label{sec:experimental}

\subsection{Results and data availability}
\label{subsec:results}
\label{subsec:verification_protocol}

For each integer $n$, the program searches over positive nonsquare integers
$d$ as in Algorithm~\ref{alg:main}; if $n\equiv 4\pmod 9$, it applies the
same search to $-n$ and negates the resulting four cubes.  We generated a
public certificate data set for every integer $0\leq n\leq 10^9$, with each
row checked by exact integer arithmetic.  Solutions for negative integers
$-n$ are obtained by taking the negatives of the four entries, and some small
examples are listed in Table~\ref{tab:solutions}.
The associated code and data are publicly available on Hugging Face at
\url{https://huggingface.co/datasets/}\texttt{kzoacn/sum4cubes}.
...
\input{figures/fig_scatter.tex}

\input{figures/fig_quantiles.tex}

\input{figures/fig_average.tex}
...
parity-incompatible solutions, which led us to state the parity conditions
explicitly and to enumerate the relevant Pell representatives in the final
implementation.
...
described in Section~\ref{subsec:verification_protocol} verifies only the resulting
...
def solve(n, LIMIT=10**8, return_d=False):
    """
    Solve x1^3 + x2^3 + x3^3 + x4^3 = n by the Pell family.
    """
    n = ZZ(n)
    if n % 9 == 4:
